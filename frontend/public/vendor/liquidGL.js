...
(() => {
        ...
        try {
          ...
          const ignoreElementsFunc = (element) => {
            ...
            return (
              ...
              element.closest("[data-nextjs-toast]")
            );
          };
  ...
  window.liquidGL = function (userOptions = {}) {
    ...
    const instances = Array.from(nodeList).map((el) =>
      ...
      if (options.liveSnapshot && renderer.captureSnapshot) {
        if (renderer._liveSnapshotTimer) {
          clearInterval(renderer._liveSnapshotTimer);
        }

        renderer._liveSnapshotTimer = setInterval(() => {
          renderer.captureSnapshot();
        }, options.liveSnapshotInterval || 900);
      }
