...
export default function CourseModal({
  ...
  useEffect(() => {
    ...
    setError(null);
    void getUsers("professor").then(setProfessors).catch(() => setProfessors([]));
    ...
    if (course) {
      setForm({
        ...
        professor_user: course.professor_user,
      });
    } else {
      setForm(initialForm);
    }
    ...
    return (
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Field label="Assigned professor">
            <Select
              value={form.professor_user ?? ""}
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  professor_user: event.target.value
              ...
              className="h-12 w-full rounded-[18px] border border-white/60 bg-white/55 px-4 text-slate-950 outline-none transition focus:bg-white/80"
            >
              <option value="">No assigned professor</option>
